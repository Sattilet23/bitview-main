...
<div id="vm-layout-left">
    <ol class="vm-vertical-nav">
        ...
        <li><a class="selected" href="/my_videos"><?= $LANGS['uploaded'] ?></a></li>
...
<div id="vm-layout-right">
    ...
    <div id="vm-video-actions-bar" class="goog-scrollfloater">
        ...
        <form method="get" id="vm-videos-search-form">
            <input name="search" value="<?php if (isset($_GET['search'])) { echo $_GET['search']; } ?>" size="25">
            <button type="button" id="vm-myvideos-search-btn" onclick=";return false;" class=" yt-uix-button"><span class="yt-uix-button-content"><?= $LANGS['search'] ?></span></button>
        </form>
        <input type="checkbox" id="vm-video-select-all" onchange="checkAll(this);">
            ...
            <button type="button" id="vm-playlist-delete-videos" onclick="deleteMyVideos();return false;" class=" yt-uix-button" disabled=""><span class="yt-uix-button-content"><?= $LANGS['delete'] ?></span></button>
        ...
        &nbsp;
        <span id="vm-num-videos-shown"><?= $_USER->Info['videos'] + $_USER->Info['private_videos'] ?></span> <?php if ($_USER->Info['videos'] + $_USER->Info['private_videos'] + $_USER->Info['converting_videos'] != 1) { echo $LANGS['plvideoamount']; } else { echo $LANGS['plvideoamountsingular']; } ?>
...
<a href="/watch?v=<?= $Video['url'] ?>" class="video-thumb ux-thumb-94" id="video-thumb-<?= $Video['url'] ?>"><span class="img"><img src="<?= $Video['thumb'] ?>" onclick="ieThumbEvent(event, this); " title="<?= $Video['title'] ?>"></span><span class="video-time"><?= timestamp($Video["length"]) ?></span></a>
            ...
            <span class="vm-video-info">
                <span class="yt-uix-button-group"><button type="button" href="/edit_video?v=<?= $Video['url'] ?>" onclick=";window.location.href=this.getAttribute('href');return false;" class="start vm-playlist-edit-video yt-uix-button yt-uix-button-short"><span class="yt-uix-button-content"><?= $LANGS['editinfo'] ?></span></button><button href="/insight?v=<?= $Video['url'] ?>" type="button" class=" yt-uix-button yt-uix-button-short" onclick=";window.location.href=this.getAttribute('href');return false;"><span class="yt-uix-button-content"><?= $LANGS['insight'] ?></span></button><button type="button" onclick="dropdown2(this);return false;" onfocusout="dropdown(this);" class="end yt-uix-button yt-uix-button-short yt-uix-button-arrowbutton"> <img class="yt-uix-button-arrow" src="/img/pixel.gif" alt=""><ul style="min-width: 103px;left: 403px;top: 429.258px;" class="yt-uix-button-menu yt-uix-button-menu-short hid"><li style="border: 0;height: auto;"><span href="/videos/<?= $Video['file_url'] ?><?php if ($Video['hd']): ?>.720<?php endif ?>.mp4" class=" yt-uix-button-menu-item" onclick=";window.location.href=this.getAttribute('href');return false;"><?= $LANGS['downloadmp4'] ?></span></li></ul></button></span>
                <?php if ($Video['privacy'] == 1): ?>
                <span class="vm-video-status"><img class="vm-sprite vm-video-privacy vm-public" src="/img/pixel.gif"> <?= $LANGS['public'] ?></span>
                <?php elseif ($Video['privacy'] == 3): ?>
                <span class="vm-video-status"><img class="vm-sprite vm-video-privacy vm-unlisted" src="/img/pixel.gif"> <?= $LANGS['unlisted'] ?></span>
                <?php else: ?>
                <span class="vm-video-status"><img class="vm-sprite vm-video-privacy vm-private" src="/img/pixel.gif"> <?= $LANGS['private'] ?></span>
                <?php endif ?>
                <?php if ($Video["status"] != 2) : ?><span class="vm-video-status" style="color: #666"><?php if ($Video["status"] == -1) : ?><?= $LANGS['failed'] ?><?php elseif ($Video["status"] == 1) : ?><?= $LANGS['converting'] ?><?php else : ?><?= $LANGS['uploading'] ?><?php endif ?></span><?php endif?><?php if ($Video['age_restricted']): ?> | Age-restricted<?php endif?>
        </li>
    <?php endforeach ?>
    <?php else: ?>
