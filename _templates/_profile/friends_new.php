...
<?= $LANGS['friends'] ?>
					(<span name="channel-box-item-count"><?php if ($LANGS['numberformat'] == 1): ?><?= number_format($_PROFILE->Info["friends"]) ?><?php else: ?><?= ($_PROFILE->Info["friends"]) ?><?php endif ?></span>)
		</div>
...
<?php if ($Friends) : ?>
	<div style="zoom:1;margin: 0 -12px">
				<?php foreach ($Friends as $Subscriber) : ?>
