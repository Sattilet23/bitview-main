...
<div class="inner-box" id="user_hubber_links">
	...
	<?php if (!$_PROFILE->Info["channels_title"]): ?><?= $LANGS['featuredchannels'] ?><?php else: ?><?= $_PROFILE->Info["channels_title"] ?><?php endif ?>
		...
		<?php if ($_PROFILE->Username == $_USER->Username): ?><div class="updown_arrows "><img id="user_hubber_links-left-arrow" src="/img/pixel.gif" class="module-left-arrow disabled"><img id="user_hubber_links-up-arrow" src="/img/pixel.gif" class="module-up-arrow <?php if ($Modules_L[0] == "otherchannels"): ?> disabled<?php endif ?>" onclick="move_up('hubber_links');return false"><img id="user_hubber_links-down-arrow" src="/img/pixel.gif" class="module-down-arrow <?php if (end($Modules_L) == "otherchannels"): ?>disabled<?php endif ?>" onclick="move_down('hubber_links');return false"><img id="user_hubber_links-right-arrow" src="/img/pixel.gif" class="module-right-arrow" onclick="move_right('hubber_links')"></div><?php endif ?>
...
<div class="edit_info">
    ...
    <div class="edit_info">
        ...
        <div style="float:left;text-align:right;">
                    <input type="text" name="fc_title" maxlength="60" style="width: 245px;" id="fc_title" maxlength="128" value="<?php if (!$_PROFILE->Info["channels_title"]): ?><?= $LANGS['featuredchannels'] ?><?php else: ?><?= $_PROFILE->Info["channels_title"] ?><?php endif ?>">
        </div>
        <div style="float:left;margin-top: 6px;"><?= $LANGS['featuredchannelsdesc'] ?></div>
        <div style="float:left;text-align:right;">
                    <textarea maxlength="125" type="text" name="fc" id="fc" class="edit_text" style="width: 245px;height: 80px;resize: none;margin-top: 2px;margin-bottom: 6px;"><?= $_PROFILE->Info['channels'] ?></textarea>
        </div>
        </div>
...
<?= $LANGS['or'] ?>
	...
	<div id="user_hubber_links-body">
		<table border="0" cellspacing="0" cellpadding="0" width="100%">
				<tbody><tr><td valign="top" width="48%" style="padding-bottom: 15px;">
				<a href="/user/<?= $fc[$i] ?>"><?= displayname($fc[$i]); ?></a><br>
...
<div class="inner-box" id="user_hubber_links">
	...
	<?php if (!$_PROFILE->Info["channels_title"]): ?><?= $LANGS['featuredchannels'] ?><?php else: ?><?= $_PROFILE->Info["channels_title"] ?><?php endif ?>
		...
		<?php if ($_PROFILE->Username == $_USER->Username): ?><div class="updown_arrows"><img id="user_hubber_links-left-arrow" src="/img/pixel.gif" class="module-left-arrow disabled"><img id="user_hubber_links-up-arrow" src="/img/pixel.gif" class="module-up-arrow <?php if ($Modules_L[0] == "otherchannels"): ?> disabled<?php endif ?>" onclick="move_up('hubber_links');return false"><img id="user_hubber_links-down-arrow" src="/img/pixel.gif" class="module-down-arrow <?php if (end($Modules_L) == "otherchannels"): ?>disabled<?php endif ?>" onclick="move_down('hubber_links');return false"><img id="user_hubber_links-right-arrow" src="/img/pixel.gif" class="module-right-arrow" onclick="move_right('hubber_links');return false"></div><?php endif ?>
		<div style="float:right;zoom:1;_display:inline;white-space:nowrap">
			<?php if ($_PROFILE->Username == $_USER->Username): ?>
                    <a class="channel-cmd" href="#" onclick="document.getElementById('user_hubber_links').classList.add('edit_mode');return false;" id="user_hubber_links_edit_link"><?= mb_strtolower((string) $LANGS['edit'])  ?></a>
                <?php endif ?>
