...
<div style="width: 958px; text-align: left;" class="sub_box">
	<div>
		...
		<span><?= $LANGS['channelsubscribers'] ?> (<?= $_PROFILE->Info["subscribers"] ?>)</span>
		<div class="clear">
	</div>
</div>
	</div>
		...
		<div class="basicBoxes" style="width:958px">
			<div class="BoxesInnerOpacity">
				...
				<?php foreach ($Users_Subscribers as $Friend) : ?>
...
<div style="float:left;margin:8px;">
		...
		<div class="title">
				...
				<a href="/user/<?= $Friend["username"] ?>" style="font-size: 11px;"><?= displayname($Friend["username"]) ?></a>
						...
						<?php endforeach ?>
						<?php else : ?>
							<div style="font-size:15px;color:#<?= $_PROFILE->Info["c_normal_font"] ?>;text-align:center;padding:22px 0 29px"><?= $LANGS['nosubscribers'] ?></div>
						<?php endif ?>
		</div>
				</div>
